...
def forget(vae, args, config, device, optimizer, labels_to_forget):
    ...
    for step in tqdm(range(0, args.n_iters)):
        ...
        c_remember = torch.from_numpy(np.random.choice(labels_to_forget, size=args.batch_size)).to(device)
...
def train_sa_vae(args, config, labels_to_forget):
    ...
    forget(vae, args, config, device, optimizer, labels_to_forget)
