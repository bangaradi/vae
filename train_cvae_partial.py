...
def sample(step):
    ...
    with torch.no_grad():
        ...
        z = torch.randn((args.n_vis_samples, config.z_dim)).to(device)
        # c = torch.from_numpy(np.random.choice(args.labels_to_learn, size=args.n_vis_samples)).to(device) 
        # one digit in one row
        c = torch.from_numpy(np.array([i for i in args.labels_to_learn for _ in range(args.n_vis_samples//len(args.labels_to_learn))])).to(device)
        c = F.one_hot(c, 10)

        out = vae.decoder(z, c).view(-1, 1, 28, 28)
        
        grid = make_grid(out, nrow = args.n_vis_samples//len(args.labels_to_learn))
        print("inside sample step : ", config.log_dir, "step_" + str(step) + ".png")
